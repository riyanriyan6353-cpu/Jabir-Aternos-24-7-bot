...
function tell(username, message) {
  ...
  // Geyser/Aternos Bedrock usernames are commonly represented with a leading dot.
  // Incoming Mineflayer whisper usernames may omit that dot, so try the Geyser
  // form first for this server, then fall back to the raw username.
  const raw = String(username || '').trim();
  const recipients = raw.startsWith('.') ? [raw] : [`.${raw}`, raw];

  for (const recipient of recipients) {
    try {
      bot.chat(`/msg ${recipient} ${safe}`);
      return;
    } catch (_) {}
  }
}
...
function findPlayerEntity(username) {
  if (!bot) return null;
  const raw = String(username || '').trim();
  const candidates = raw.startsWith('.') ? [raw, raw.slice(1)] : [raw, `.${raw}`];
  for (const name of candidates) {
    const player = bot.players[name];
    if (player?.entity) return { username: name, entity: player.entity };
  }
  const target = candidates.find((candidate) =>
    Object.keys(bot.players || {}).some((key) => key.toLowerCase() === candidate.toLowerCase())
  );
  if (target) {
    const player = bot.players[target];
    if (player?.entity) return { username: target, entity: player.entity };
  }
  return null;
}
...
async function deliverItem(targetUsername, itemName, amount) {
  ...
  const target = bot.players[trackedUsername]?.entity;
  ...
  try {
    ...
    while (Date.now() < deadline) {
      const current = bot.players[trackedUsername]?.entity;
